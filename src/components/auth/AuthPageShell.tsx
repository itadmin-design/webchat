...
const HEADLINE_TEXT = "Добро пожаловать";

type AuthTab = "magic-link" | "login" | "apply";

export function AuthPageShell({ defaultTab = "magic-link" }: { defaultTab?: AuthTab }) {
  useUtmCapture();
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      <div className="lg:w-1/2 bg-gradient-to-br from-black/5 to-muted/50 flex flex-col px-10 lg:px-16 py-10 lg:py-0">
        ...
        {/* Headline with per-character drop-in animation */}
        <div className="flex-1 flex flex-col justify-center py-12 lg:py-0">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-foreground leading-[1.1] flex flex-wrap">
            {HEADLINE_TEXT.split(" ").map((word, wi, words) => {
          ...
          <motion.p
            ...
            transition={{
              duration: 0.7,
              delay: HEADLINE_TEXT.length * 0.05 + 0.3,
              ease: [0.22, 1, 0.36, 1],
            }}
          >
            ...
            <Tabs value={tab} onValueChange={setTab} className="w-full">
              <CardHeader className="pb-0">
                <TabsList className="w-full grid grid-cols-3 h-10">
                  ...
                  <TabsTrigger value="apply" className="text-xs sm:text-sm">Стать клиентом</TabsTrigger>
                </TabsList>
              </CardHeader>
